...
# Take a bank of n lights
# loop over them n times
#   turn on the lights at step i, starting at i
#   each iteration, increase i by 1
# end loop
# select all lights that are on
# return an array, with the numbers of all the lights that are left on.

def thousand_lights(n)
  lights = Array.new(n, true)
  (2..n).each do |i|
    ((i - 1)..(n - 1)).step(i).each do |j|
      lights[j] = !lights[j]
    end
  end
