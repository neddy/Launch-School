...
def each_cons(collection, step)
  collection.each_index do |idx|
    break if idx > collection.size - step
    current_set = if step == 1
                    collection[idx]
                  else
                    collection[idx, step]
                  end
...
# Solution using splat
puts "Solution using splat."
def each_cons(collection, step)
  collection.each_index do |idx|
    break if idx > collection.size - step
    yield(*collection[idx, step])
  end
  nil
end
