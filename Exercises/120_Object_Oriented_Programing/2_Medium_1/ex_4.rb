...
# Requirements
# Define queue length
# add new objects to end of queue
# if cue length is longer than designated length after adding item to queue, dicard oldest element
...
class CircularQueue
  ...
  def enqueue(element)
    @queue << element
    @queue.shift if @queue.size > @length
    element # Not necessary, but stops the method from returning the removed item. Could also be nil
  end
