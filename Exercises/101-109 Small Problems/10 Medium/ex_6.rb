...
def minilang(program)
  ...
  program.split.each do |token|
    case token
    when token.to_i.to_s
      register = token.to_i
    when "PUSH"
      stack << register
    when "ADD", "SUB", "MULT", "DIV", "MOD"
      if stack.empty?
        return error("Emptyt stack detected when trying to #{token}.")
      else
        register = [register, stack.pop].reduce(OPERATIONS[token])
      end
    when "POP"
      if stack.empty?
        return error("Emptyt stack detected when trying to #{token}.")
    ...
    when "PRINT"
      ...
      return error("Invalid token detected: #{token}")
    end
  end
  nil
end
